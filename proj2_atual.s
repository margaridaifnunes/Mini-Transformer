...
# A condição a3 == a5 é necessária para a multiplicação ser válida.
# ===========================================================================
matrix_multiply:
    addi sp, sp, -44
    sw ra,  0(sp)
    ...
    # Guardar argumentos em registos s (ficam seguros através dos jal ra, dot)
    mv s0, a0           # ponteiro output (avança 4 por cada resultado escrito)
    mv s1, a1           # ponteiro linha atual de A (E)
    mv s2, a2           # contador de linhas restantes de A
    mv s8, a3           # #cols de A = #rows de B = tamanho do dot product
    ...
    # stride de uma coluna de B em bytes = #cols_B * 4
    slli s4, s5, 2      # s4 = stride coluna B = s5 * 4
 
loop_matrix:
    beqz s2, matrix_mult_end    # acabaram as linhas de A
    addi s2, s2, -1             # decrementa contador de linhas
 
    mv s3, s5                   # reset contador de colunas de B (#cols B)
    mv s7, s9                   # s7 = ponteiro início de B (coluna 0, linha 0)
    sub s7, s7, s4              # recuar 1 coluna (será incrementado no início de loop_column)
 
loop_column:
    beqz s3, next_line_matrix   # acabaram as colunas de B
    addi s3, s3, -1             # decrementa contador de colunas
    add s7, s7, s4              # avançar para a próxima coluna de B (4 bytes = 1 int por linha)
 
    # Extrair coluna s7 de B para COLUMN_VECTOR
    # Cada elemento consecutivo da mesma coluna está separado por stride_col = s4 bytes
    la a2, COLUMN_VECTOR        # a2 = ponteiro no COLUMN_VECTOR
    mv t0, s7                   # t0 percorre a coluna de B
    mv t2, s8                   # contador = #linhas de B = s8
 
loop_value:
    beqz t2, apply_dot
    lw t4, 0(t0)                # elemento da coluna de B
    sw t4, 0(a2)                # guarda no COLUMN_VECTOR
    add t0, t0, s4              # próxima linha da mesma coluna de B
    addi a2, a2, 4
    addi t2, t2, -1
    j loop_value
 
apply_dot:
    mv a1, s1                   # linha atual de A
    la a2, COLUMN_VECTOR        # coluna extraída de B
    mv a3, s8                   # tamanho = #cols de A = #rows de B
    jal ra, dot                 # resultado em a1
 
    sw a1, 0(s0)                # escrever resultado na matriz output
    addi s0, s0, 4              # avançar ponteiro output
    j loop_column
