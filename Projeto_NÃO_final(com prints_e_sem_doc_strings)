...
main:
    ...
    sw a1, 0(t0)                 # dá store do tamnho do vocabulário   
    ###########################################################################
    ...
    sw a3, 0(t2)                    # guardar o nº de tokens para select_vector
    li a1, 3
    li a2, 4
    jal ra, print_matrix
    ###########################################################################
    ...
    ###########################################################################
    la a1, SCORES_VECTOR
    lw a2, INPUT_TOTAL_TOKENS        # array lenght = nº de tokens
    ...
    lw a2, INPUT_TOTAL_TOKENS  # nº de linhas do vocabulário (= nº de tokens)
...
decide_next_token:
    ...
    loop_embeding:
        ...
        mv a1, s0        # apontador para a linha V[1]
        mv a2, s1        # apontador para o embeding
        li a3, 4
        li a4, 1
        jal ra, dot
        #bnez a0, salta
        slli t0, s3, 2    # calcula o indice
        add t0, s4, t0    # Adiciona ao ponteiro original
        sw a1, 0(t0)      # salva num vetor 
        addi s1, s1, 16   # vai para a próxima linha dos embedings
        addi s3, s3, 1    # incrementa mais 1 iteração
        j loop_embeding
